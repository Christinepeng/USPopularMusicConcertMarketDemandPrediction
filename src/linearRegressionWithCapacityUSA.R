...
soldout_capacity_name = "sold_out_80:capacity"

# find out average capacity for each state
state_df_mean_capacity = df %>%
    group_by(state) %>%
    summarise(mean_capacity = mean(capacity)) %>%
    mutate(state = tolower(state))

# Case 2: sold_out_80 == 1 and capacity is presented
for (genre_name in gerne_name_list) {
    ...
    { # special case for Alabama
        ...
        state_ave_price_df = data.frame(
            ...
            ave_price = model$coeff["(Intercept)"] 
                        ...
                        + model$coeff[soldout_capacity_name] * state_df_mean_capacity$mean_capacity[state_df_mean_capacity$state == "alabama"]
        )
    }
    for (state_name in state_name_list) {
        ...
        state_ave_price_df = rbind(state_ave_price_df, data.frame(
            ...
            ave_price = model$coeff["(Intercept)"] 
                        ...
                        + model$coeff[soldout_capacity_name] * state_df_mean_capacity$mean_capacity[state_df_mean_capacity$state == std_state_name]
        ))
    }
    ...
    ggsave(sprintf("../res/w_capacity_w_soldout/map_us_%s.png", std_genre_name), plot=p, width=10, height=6)
}
